...
sub itemset_profile_template {
    ...
    for my $tmpl_id (@tmpls) {
        ...
        MT->log( {
                   blog_id => $app->blog->id,
                   message => "Profiling template: " . $tmpl->name
                 }
        );
...
sub results {
    ...
    if ( $tmpl->type eq 'category' ) {
        ...
        my $map = MT->model('templatemap')
          ->load( { template_id => $tmpl->id, is_preferred => 1 } );
    ...
    if ( defined $archive_type ) {
        MT->log( {
                   blog_id => $app->blog->id,
                   message => 'Initializing: ' . $archive_type
                 }
        );
        ...
        if ( $t->date_based ) {
            ...
            my $entries = sub {
                $t->dated_group_entries( $ctx, $archive_type, $ts_start );
            };
    ...
    my $param ||= {};
    return $app->error( "Builder error: " . $builder->errstr )
      if $builder->errstr;
